...
def run_stock_update_pipeline(file_path: Path, databricks_volume_path: str, cli_profile: str):
    """
    Orquesta el proceso de pre-procesamiento y carga para el archivo de stock.
    """
    ...
    # --- PASO 1: EXTRACT ---
    logger.info("--> Paso 1: EXTRACCIÓN - Leyendo archivo Excel de stock...")
...
def main():
    """
    Función principal para procesar el archivo de stock.
    """
    logger.info("=================================================================")
    logger.info("--- INICIANDO PIPELINE DE ACTUALIZACIÓN DE STOCK ---")
    logger.info("=================================================================")
    ...
    try:
        logger.info(f"Verificando archivo de stock en: {stock_file_path}")
        if not stock_file_path.exists():
            logger.error(f"El archivo de stock no existe: {stock_file_path}")
            return 1

        logger.info(f"Archivo de stock encontrado. Iniciando procesamiento.")
        if run_stock_update_pipeline(stock_file_path, DATABRICKS_VOLUME_PATH, CLI_PROFILE):
            logger.info("=================================================================")
            logger.info("--- PIPELINE FINALIZADO. Archivo de stock procesado exitosamente. ---")
            logger.info("=================================================================")
        else:
            logger.error("=================================================================")
            logger.error("--- PIPELINE FINALIZADO CON ERRORES. No se pudo procesar el archivo de stock. ---")
            logger.error("=================================================================")
            return 1
