    ...
    try:
        ...
        if file_path.suffix.lower() == ".csv":
            ...
            try:
                ...
                logger.warning(
                    "La lectura con UTF-8 falló. Reintentando con codificación 'latin1'."
                )
            ...
            if df is not None:
                ...
                if rows_dropped > 0:
                    logger.info(
                        f"Se eliminaron {rows_dropped} filas completamente en blanco del archivo Excel."
                    )
        else:
        ...
        logger.error(
            f"Error de Extracción: No se encontró el archivo en la ruta local: {file_path}"
        )
        return None, None
